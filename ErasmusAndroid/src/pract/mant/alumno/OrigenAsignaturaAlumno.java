...
/**
 * Interfaz de usuario donde el alumno puede seleccionar
 * las asignaturas que está impartiendo en la actualidad
 * y si quiere convalidarlas a la hora de solicitar
 * una beca ERASMUS
 * 
 */
public class OrigenAsignaturaAlumno extends Activity {
	...
	/**
	 * Carga la información contenida en las estructuras de datos de la clase
	 * en la interfaz para su tratamiento
	 * 
	 */
	public void cargarLista(){
		if(asignaturas != null){
			...
			AdaptadorAsignaturasLocales adaptador = new AdaptadorAsignaturasLocales(this, asignaturas);
	...
	/**
	 * Evento que se lanza cuando el usuario pulsa sobre el botón
	 * volver para retroceder a la pantalla anterior.
	 * 
	 * @param v Botón que envía el evento
	 */
	...
	/**
	 * Evento que se lanza cuando el usuario pulsa sobre el botón
	 * finalizar para enviar los datos a su matrícula.
	 * 
	 * @param v Botón que envía el evento
	 */
	...
	/**
	 * Clase para obtener la lista de asignaturas a las que un alumno se
	 * puede matricular en base a su titulación
	 *
	 */
	private class aTaskObtenerAsignaturas extends AsyncTask <Void, Void, Void> {
		...
		/**
		 * Constructor de la clase. Se inicializa en él la variable para
		 * obtener datos del alumno.
		 * 
		 * @param session Objeto para gestionar la información del alumno.
		 */
		public aTaskObtenerAsignaturas(SessionManager session){
			this.session = session;
		}
		
		private ArrayAsignaturas obtenerAsignaturas() throws IOException, XmlPullParserException{
		...
		protected Void doInBackground(Void... params) {
			...
			try{
				...
				//En este punto tenemos ya todas las asignaturas a las que el alumno puede acceder
				// así que se procede a cargarlas en un formato apropiado para el array adapter
				...
				for(ComplexAsignatura actual : respuesta.getAsignaturas()){
					...
					AsignaturaLocal wrapper = new AsignaturaLocal(_nombre, _estado, _creditos, _id, _coordinador, false);
